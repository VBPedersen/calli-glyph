...
pub fn execute_command(app: &mut App, command: Command) -> Result<(), CommandError> {
    match command {
        ...
        Command::Help { args, flags } => {
            // TODO: Show help popup or render help screen
            if let Err(e) = commands::help::help_command(app, args, flags) {
                return Err(e);
            }
            Ok(())
        }
        ...
        Command::Lang { args, flags } => {
            if let Err(e) = commands::lang::lang_command(app, args, flags) {
                return Err(e);
            }
            Ok(())
        }
