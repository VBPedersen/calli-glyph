...
pub fn save_command(app: &mut App, args: Vec<String>, flags: HashSet<CommandFlag>) -> Result<(), CommandError> {
    ...
    if let Err(e) = app.save_to_path(path) {
        Err(CommandError::ExecutionFailed(format!("failed to save file: {}", e)))
    } else {
        Ok(())
    }
