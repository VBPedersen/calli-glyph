...
mod app_editor_tests {
    ...
    fn test_write_char_in_editor_with_selected_text_multiple_lines() {
        ...
        app.text_selection_start = Option::Some(CursorPosition{ x: 6, y: 1 });
        app.text_selection_end = Option::Some(CursorPosition{ x: 13, y: 2 });
        app.cursor_x = 6;
        app.write_all_char_in_editor('W');
        assert_eq!(app.editor_content[0], "test");
        assert_eq!(app.editor_content[1], "Hello W");
        assert_eq!(app.editor_content[2], "land");
        assert_eq!(app.cursor_x, 7);
    }
    ...
    fn test_backspace_in_editor_text_is_selected() {
        ...
        assert_eq!(app.editor_content, vec!["Hello "]);
